...
// проверяем возможные шаги пешки
...
// проверяем шаги выбранной пешки
...
// разрешаем ходить королю в обратную сторону
...
// выделяем фигуру зеленым цветом при выделении
...
// даёт возможность выделить пешку, если есть возможные ходы
...
function makeMove(number) { // функция, отвечающая за передвижение
...
function changeData(indexOfBoardPiece, modifiedIndex, removePiece) { // функция, отвечающая за становление королём и удаление пешек
    board[indexOfBoardPiece] = null;
    board[modifiedIndex] = parseInt(selectedPiece.pieceId);
    if (turn && selectedPiece.pieceId < 12 && modifiedIndex >= 57) {
        document.getElementById(selectedPiece.pieceId).classList.add("king") // становимся королем
    }
    ...
    if (removePiece) {
        ...
        if (turn && selectedPiece.pieceId < 12) {
            ...
            blackScore-- // уменьшаем количество пешек черной команды
        }
        if (turn === false && selectedPiece.pieceId >= 12) {
            cells[removePiece].innerHTML = "";
            redScore-- // уменьшаем количество пешек красной команды
        }
    }
    resetSelectedPieceProperties();
    removeCellonclick();
...
function checkForWin() {
    if (blackScore === 0) {
        ...
        for (let i = 0; i < redTurnText.length; i++) {
            ...
            redTurnText[i].textContent = "Красные победили!";
        }
    } else if (redScore === 0) {
        divider.style.display = "none";
        for (let i = 0; i < blackTurntext.length; i++) {            
            ...
            blackTurntext[i].textContent = "Силы тьмы победили!";
        }
    }
    changePlayer();
}

// переключаем ход игроков
